...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBCR.db",False)
End Sub
...
Private Sub WebSocket_Connected (WebSocket1 As WebSocket)
	...
	Log(ws.Session.GetAttribute("Pais"))
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagecr"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagecr"))

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messagecr")

	
	If message.Contains("Placa") Or message.Contains("Chasis") Or message.Contains("Motor") Or message.Contains("VIN") Then
		enviarAI(message)
	End If
	
	If message.Contains("Nombre") Or message.Contains("Apellido") Or message.Contains("Identidad") Or message.Contains("Nacimiento") Then
...
Sub ProcessList(inputList As List)
	For Each item As Map In inputList
		...
		Dim anio As String = ExtractValue(text, "Año:", "Marca:")
		Dim color As String = ExtractValue(text, "Marca:", "Chasis:")
...
Sub enviarAI(message As String)
	...
	Dim partsMap As Map
	partsMap.Initialize
	partsMap.Put("text", $" del siguiente bloque de texto necesito que me identifiques y ordenes los siguientes valores MOTOR, PLACA, AÑO, MARCA, CHASIS algunos ejemplos
...
Sub enviarAINombre(message As String)
	...
	recuerda unicamente Nombre concatenado con el apellido y Numero de identidad no mas datos. si la cadena contiene N°: toma ese numero como identidad: ${message} "$)
