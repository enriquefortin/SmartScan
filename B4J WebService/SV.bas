...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBSV.db",False)
End Sub
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagesv"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagesv"))
	

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messagesv")
	
	If message.Contains("MOTOR") Or message.Contains("MODELO") Or message.Contains("COLOR") Or message.Contains("CHASIS") Then
		enviarAI(message)
	End If
	
	If message.Contains("Nombre") Or message.Contains("Identidad") Or message.Contains("ID number") Or message.Contains("Surname") Then
...
Sub ProcessList(inputList As List)
	For Each item As Map In inputList
		...
		Dim motor As String = ExtractValue(text, "Motor:", "VIN:")
		Dim placa As String = ExtractValue(text, "VIN:", "Año:")
...
Sub enviarAI(message As String)
	...
	Dim partsMap As Map
	partsMap.Initialize
	partsMap.Put("text", $" del siguiente bloque de texto necesito que me identifiques y ordenes los siguientes valores MOTOR, VIN, AÑO, COLOR, CHASIS algunos ejemplos
