...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBCO.db",False)
End Sub
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messageco"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messageco"))

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messageco")

	
	If message.Contains("REGISTRO") Or message.Contains("MARCA") Or message.Contains("LINEA") Or message.Contains("CLASE") Then
		enviarAI(message)
	End If
	
	If message.Contains("NOMBRES") Or message.Contains("APELLIDOS") Or message.Contains("NUMERO") Or message.Contains("CEDULA") Or message.Contains("PROPIETARIO")  Or message.Contains("IDENTIFICACION")  Or message.Contains("IDENTIFICACIÓN") Then
...
Sub ProcessList(inputList As List)
	For Each item As Map In inputList
		...
		Dim motor As String = ExtractValue(text, "Motor:", "Marca:")
		Dim placa As String = ExtractValue(text, "Marca:", "Año:")
		Dim anio As String = ExtractValue(text, "Año:", "Color:")
		Dim color As String = ExtractValue(text, "Color:", "Serie:")
		Dim chasis As String = ExtractValue(text, "Serie:", "")
		Log(motor)
...
Sub enviarAI(message As String)
	...
	Dim partsMap As Map
	partsMap.Initialize
	partsMap.Put("text", $" del siguiente bloque de texto necesito que me identifiques y ordenes los siguientes valores MOTOR, MARCA, AÑO, COLOR, SERIE algunos ejemplos
	 de motor son los siguientes (45386047, YD25776714P, MGC20597, CE7AN29R9052) algunos ejemplos de numeros de marca son los siguientes TOYOTA
	  el año siempre toma el que no encuetres bajo el texto año sino el siguiente el color pueden ser dos ejemplo AZUL-BLANCO o tambien puede ser solo uno ejemplo AZUL
	   toma en cuenta que el numero del motor siempre tendra menos o igual tamaño de caracteres que el numero de serie favor limita la respuesta unicamente a los datos
		...
		de serie son los siguientes (1FV6HLAA7VL714763, 3N6CD33B2ZK463860, JTEBR3FJ60K277588): ${message} "$)
