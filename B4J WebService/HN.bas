...
Sub Class_Globals
	...
	Private  txtmotor, txtplaca, txtanio, txtcolor, txtchasis, txtfecha As JQueryElement
...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBHN.db",False)
...
Private Sub gebd_Click(params As Map)
	...
	mysql.ExecNonQuery2("insert into InfoDB values (?,?,?,?,?,?,?,?,?,?) ", _
		Array As Object(Null, txtnombre.GetVal.value,txtidentidad.GetVal.Value,txtfecha.GetVal.Value,txtmotor.GetVal.Value,txtplaca.GetVal.value, txtanio.GetVal.Value, txtcolor.GetVal.Value, _
		 txtchasis.GetVal.Value,"HN"))
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagehn"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagehn"))	

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messagehn")
	...
	If message.Contains("NOMBRE") Or message.Contains("IDENTIDAD") Or message.Contains("FORENAME") Or message.Contains("IDENTIFICACION") Then
...
Sub enviarAINombre(message As String)
	...
	Dim partsMap As Map
	partsMap.Initialize
	partsMap.Put("text", $" del siguiente bloque de texto necesito que me identifiques y ordenes los siguientes valores Nombre, Numero de identificacion y fecha de nacimiento el nombre puedes encontrarlo 
...
Sub ProcessListNombre(inputList As List)
	For Each item As Map In inputList
		...
		Dim identidad As String = ExtractValue(text, "Identidad:", "\Fecha:")
		Dim fecha As String = ExtractValue(text, "Fecha:", "\n")
