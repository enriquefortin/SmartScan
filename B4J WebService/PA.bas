...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBPA.db",False)
End Sub
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagepa"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messagepa"))

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messagepa")

	
	If message.Contains("MOTOR") Or message.Contains("PLACA") Or message.Contains("COLOR") Or message.Contains("CHASIS") Or message.Contains("PUERTAS") Then
		enviarAI(message)
	End If
	
	If message.Contains("NOMBRE") Or message.Contains("NOMBRE USUAL") Or message.Contains("SEXO") Then
