...
Public Sub Initialize
	partes.Initialize
	Main.sql1.InitializeSQLite(File.DirApp & "/DB", "InfoDBNI.db",False)
End Sub
...
Private Sub jqm_Click(params As Map)
	...
	Log(Main.msj.Get(txtiddelmovil.GetVal.Value&"messageni"))
	Log(txtiddelmovil.GetVal.Value)
	txtmensaje.SetHtml("")
	Log("CLICK")

	txtmensaje.SetText(Main.msj.Get(txtiddelmovil.GetVal.Value&"messageni"))
	

	Dim message As String = Main.msj.Get(txtiddelmovil.GetVal.Value&"messageni")
	...
	If message.Contains("Nombres") Or message.Contains("IDENTIDAD") Or message.Contains("Apellidos") Or message.Contains("Sexo") Then
...
Sub ProcessList(inputList As List)
	For Each item As Map In inputList
		...
		Dim placa As String = ExtractValue(text, "Placa:", "Vin:")
		Dim anio As String = ExtractValue(text, "Vin:", "Color:")
...
Sub sh1_ProcessCompleted (Success As Boolean, ExitCode As Int, StdOut As String, StdErr As String)
	If Success And ExitCode = 0 Then
		...
		If message.Contains("Nombres") Or message.Contains("Apellidos") Or message.Contains("Sexo") Or message.Contains("IDENTIDAD") Then
			enviarAINombre(message)
		End If
...
Sub enviarAI(message As String)
	...
	Dim partsMap As Map
	partsMap.Initialize
	partsMap.Put("text", $" del siguiente bloque de texto necesito que me identifiques y ordenes los siguientes valores MOTOR, PLACA, VIN, COLOR, CHASIS algunos ejemplos
